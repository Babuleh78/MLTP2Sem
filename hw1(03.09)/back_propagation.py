...
class MLPRegressor:
    ...
    def _forward(self, x):
        ...
        for i in range(len(self.weights) - 1):
            neiron_value = np.matmul(self.activations[-1], self.weights[i]) + self.biases[i]
            
            sigm_value = self._sigmoid(neiron_value)
            self.activations.append(sigm_value)
        
        # Для выходного нейрона используем активацию f(x)=x
        output = np.matmul(self.activations[-1], self.weights[-1]) + self.biases[-1]
    ...
    def _back(self, x, y_true, y_pred):
        ...
        for i in range(len(self.weights)-1, -1, -1):
            if i == len(self.weights) - 1: 
                ...
                grad_n = grad_a * self._sigmoid_proizv(self.activations[i + 1])
                
            grad_w = np.matmul(self.activations[i].T, grad_n / n_samples)
            grad_b = np.sum(grad_n, axis=0, keepdims=True) / n_samples
            
            grads_w.append(grad_w)
            grads_b.append(grad_b)
            
            if i > 0:
                grad_a = np.matmul(grad_n, self.weights[i].T)
